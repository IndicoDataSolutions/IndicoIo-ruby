...
describe Indico::Results do
  ...
  context '#[]' do
    it 'can take a key' do
      data = {'a' => 5, 'b' => 6, 'c' => 5.5, 'd' => 4, 'e' => 5}
      result = Indico::Results.new(data)
      expect(result['a']).to eql(5)
      expect(result['d']).to eql(4)
    end

    it 'can handle mixed-case keys' do
      data = {'Aa' => 5, 'Bb' => 6, 'Cc' => 5.5, 'Dd' => 4, 'ee' => 5}
      result = Indico::Results.new(data)
      expect(result['aa']).to eql(5)
      expect(result['Aa']).to eql(5)
      expect(result['dd']).to eql(4)
      expect(result['Dd']).to eql(4)
    end
  end
